...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x,memory,embed_spd_flow,embed_ped_flow,enc_mask,spd_mask,ped_mask):
        """
        Follow Figure 2 for connections.
        """
        m = memory
        spd=embed_spd_flow
        ped=embed_ped_flow
        ...
        x = self.sublayer[3](x, lambda x: self.spd_attn(x, spd, spd, spd_mask),
                             sublayer1=lambda x: self.src_attn(x, m, m, enc_mask),
                             sublayer2=lambda x: self.ped_attn(x, ped, ped, ped_mask))
